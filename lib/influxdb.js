...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  for (p in points) {
    ...
    // "a=1.2,21=atagher#listia.production.image.res_time.timer.count".split(/([^#]+)#(.+)/)
    // [ '',
    //  'a=1.2,21=atagher',
    //  'listia.production.image.res_time.timer.count',
    //  '' ]
    if (metricAndTags.length >= 3) {
      sanitizeMetricName = metricAndTags[2].replace(/\s+/g, '_')
          .replace(/\//g, '-')
          .replace(/[^a-zA-Z_\-0-9\.]/g, '')
      sanitizeTags = metricAndTags[1] == "" ? "": `,${metricAndTags[1]}`
      lines[totalPoint++] = `${sanitizeMetricName}${sanitizeTags} value=${points[p]['fields']['value']}`
    }
  }
  payload = lines.join("\n")
