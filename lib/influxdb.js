...
function InfluxdbBackend(startupTime, config, events) {
  ...
  if (config.influxdb) {
    ...
    self.host_v08 = config.influxdb.host_v08;
    self.port_v08 = config.influxdb.port_v08;
  }
...
InfluxdbBackend.prototype.processFlush = function (timestamp, metrics) {
  ...
  // @TODO: Remove after finishing filter metrics
  var points_v08 = [];
  ...
  for (key in counters) {
    ...
    if (key.indexOf('#') == -1) {
      points_v08.push(self.assembleEvent_v08(k, [{value: value, time: timestamp}]))
      continue
    }
...
InfluxdbBackend.prototype.httpPOST_v08 = function (points) {
  ...
  var self = this,
      ...
      //hard code pw for old influxdb 0.8
      query = {u: 'root', p: 'root', time_precision: 'ms'},
      protocolName = self.protocol == http ? 'HTTP' : 'HTTPS',
      startTime;

  self.logDebug(function() {
    return '>> About to send legacy(0.8) metrics' + JSON.stringify(points) + ">>\n"
  })
  ...
  // @TODO remove this after finishing proxying metric
  // We are hard coding the address of old influxdb 0.8 here
  var options = {
    hostname: self.host_v08, //self.host,
    port: self.port_v08,//self.port,
  ...
  req.on('response', function (res) {
    ...
    if (status !== 200) {
      self.log(protocolName + ' Error: ' + status + ' Version: 0.8');
    }
  });
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  self.logDebug(function() {
    return '>> About to send modern(0.9) metrics' + JSON.stringify(points) + ">>\n"
  })
  ...
  req.on('response', function (res) {
    ...
    if (status >= 400) {
      self.log(protocolName + ' Error: ' + status + ' Version: 0.9');
    }
  });
  ...
  for (p in points) {
    //metricname#tag1=v1,tag2=1.metrictype
    var metricAndTags = points[p]['measurement']
      .replace(/\s+/g, '_')
      .replace(/\//g, '-')
      .replace(/[^a-zA-Z_\-0-9\.#,=]/g, '')

    metricAndTags = metricAndTags.split('#')
    console.log(metricAndTags)

    if (metricAndTags.length == 1) {
      lines[totalPoint++] = `${metricAndTags[0]} value=${points[p]['fields']['value']}`
      continue
    }

    for (i in metricAndTags) {
      metricAndTags[i] = metricAndTags[i].replace(/\s+/g, '_')
          .replace(/\//g, '-')
          .replace(/[^a-zA-Z_\-0-9\.,=]/g, '')
    }

    if (metricAndTags.length == 2) {
      lines[totalPoint++] = `${metricAndTags[0]}${metricAndTags[1]} value=${points[p]['fields']['value']}`
      continue
    }

    if (metricAndTags.length >= 3) {
      sanitizeTags = metricAndTags[1] == "" ? "": `,${metricAndTags[1]}`
      lines[totalPoint++] = `${metricAndTags[0]}${metricAndTags[2]}${sanitizeTags} value=${points[p]['fields']['value']}`
    }
  }
  payload = lines.join("\n")
...
InfluxdbBackend.prototype.configCheck = function () {
  ...
  self.log('Run with InfluxDB ' + self.version)
